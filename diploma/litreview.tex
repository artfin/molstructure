...
\end{gather}
%
При предельном переходе в \eqref{part1-bn-expression2} первые два слагаемые преобразуются к дельта-функ\-ционалам, в то время как последнее слагаемое оказывается нулевым \cite{baym-quantum-mechanics}. Итак, выражение для скорости переходов оказывается следующим \cite{mcquarrie-statistical-mechanics}  
%
\begin{gather}
...
\begin{gather}
    \ket{\psi(t)} = \sum_n c_n(t) \ket{\varphi_n}, \quad c_n(t) = \braket{\varphi_n | \psi(t) }, \label{app-expansion}
\end{gather}
...
В соответствии с предположением, при $t < 0$ система находится в состоянии $\ket{\varphi_i}$, следовательно, среди коэффициентов 
...
\begin{gather}
    \ket{\psi(t)} = \sum_n c_n(t) \ket{\varphi_n} + \int c_\alpha(t) \ket{\varphi, \alpha} d\alpha.
\end{gather}
