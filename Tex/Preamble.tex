\usepackage[T3]{fontenc}

\usepackage[utf8]{inputenc}
\usepackage[russian]{babel}

% page margin
\usepackage[top=2cm, bottom=2cm, left=2cm, right=2cm]{geometry}

% AMS packages
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsthm} % theorem environment

% blackboard lettring
\usepackage{dsfont} % uppercase
\usepackage{bbm} % lowercase
\usepackage[bb=boondox]{mathalfa} % numbers

\usepackage{fancyhdr}
\pagestyle{fancy}

% modifying page layout using fancyhdr
\fancyhf{}
\renewcommand{\sectionmark}[1]{\markright{\thesection\ #1}} % adding number to section name
\renewcommand{\subsectionmark}[1]{\markright{\thesubsection\ #1}} % adding number to subsection name

\rhead{\fancyplain{}{\rightmark }} % placing the section/subsection name in the right corner of the header
\cfoot{\fancyplain{}{\thepage}} % placing a page number in the center of the footer   

\renewcommand{\headrulewidth}{1 pt} % placing horizontal rules
\renewcommand{\footrulewidth}{1 pt} % placing horizontal rules

% includegraphics
\usepackage{graphicx}

%appendices
\usepackage[toc, page]{appendix}
\usepackage{titlesec}

% tikz libraries
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows, positioning, decorations.markings}
\usetikzlibrary{fit}
\usepackage{microtype}
\usepackage{framed}
\usetikzlibrary{decorations.pathmorphing,calc,backgrounds}


% cures a bug inside titlesec package that overrides section numbering
\usepackage{etoolbox}

\makeatletter
\patchcmd{\ttlh@hang}{\parindent\z@}{\parindent\z@\leavevmode}{}{}
\patchcmd{\ttlh@hang}{\noindent}{}{}{}
\makeatother

% COMMANDS AND ENVIRONMENTS

% theorem environment
\newtheorem{mytheorem}{Теорема}[]

% blackboard symbols
% using dsfont and bbm packages 
\newcommand{\bbA}{\mathds{A}}
\newcommand{\bbI}{\mathds{I}}
\newcommand{\bbB}{\mathds{B}}
\newcommand{\bba}{\mathbbm{a}}
\newcommand{\bbb}{\mathbbm{b}}
\newcommand{\bbS}{\mathds{S}}
\newcommand{\bbE}{\mathds{E}}
\newcommand{\bbG}{\mathds{G}}
\newcommand{\bbU}{\mathds{U}}
\newcommand{\bbV}{\mathds{V}}
\newcommand{\bbO}{\mathds{O}}
\newcommand{\bbQ}{\mathds{Q}}
\newcommand{\bbzero}{\mathbb{0}}

% enumerate equations inside align* environment
\newcommand\numberthis{\refstepcounter{equation}\tag{\theequation}} 

% shifting commands
\newcommand{\vlevo}{\hspace*{-0.63cm}}
\newcommand{\vverh}{\vspace*{-0.1cm}}
\newcommand{\dn}{\! \!} % "double negative"

% round brackets
\newcommand{\lb}{\left(}
\newcommand{\rb}{\right)}

% primes
\newcommand{\dpr}{\prime \prime}

% chemistry formulas
\usepackage{mhchem}

% nice fractions
\usepackage{nicefrac}
\usepackage{xfrac}

% defines new command ddfrac that forces nominator and denominator to be displayed in displaystyle manner
\newcommand\ddfrac[2]{\frac{\displaystyle #1}{\displaystyle #2}}

\usepackage{array}

% bdmatrix environment that forces each element to be displayed in displaystyle manner
\makeatletter
\def\env@dmatrix{\hskip -\arraycolsep
  \let\@ifnextchar\new@ifnextchar
  \extrarowheight=2ex
  \array{*\c@MaxMatrixCols{>{\displaystyle}c}}}

\newenvironment{dmatrix}
  {\env@dmatrix}
  {\endarray\hskip-\arraycolsep}

\newenvironment{bdmatrix}
  {\left[\env@dmatrix}
  {\endmatrix\right]}
% and other matrix environments are similar
\makeatother

% to define "figure" environment
\usepackage{float}